...
- 從版本 1.6.0 後使用者可以自行下載更新音樂資料庫而不用依賴新版本的發布。
- ScoreViewer 左下角會顯示目前正在使用的資料庫路徑。
- 如何使用新音樂資料庫:
    - 新增檔案 `usingDB.txt` 到 `table/` 資料夾中。
    - 將新的音樂資料庫加到 `table/`, 例如 `table/MusicDatabase37_2030-02-20.json`
    - 編輯 `usingDB.txt` 內加入`MusicDatabase37_2030-02-20.json` 至第一行。
    - `table/usingDB.txt`

        ```
        MusicDatabase37_2030-02-20.json
        ```
    - 如果路徑不正確，將會使用預設路徑。檢查左下角是否顯示使用了新的音樂資料庫。
